...
// ========== MIGRATION: Python Backend Integration ==========
// If PYTHON_BACKEND_URL is set, proxy execution requests to Python backend
// Otherwise use local Claude CLI/SDK (legacy)
...
if (USE_PYTHON_BACKEND) {
  ...
  console.log(`[MIGRATION] Using Python LangGraph backend at ${PYTHON_BACKEND_URL}`);
...
const proxyToPython = async (path, method = 'GET', data = null) => {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          resolve({ status: res.statusCode, body: { error: responseData || 'Invalid JSON' } });
        }
      });
    });
    req.on('error', reject);
...
window.FRONTEND_TASK_CREATION = false;
    ...
    if (url.pathname === '/api/tasks' && req.method === 'POST') {
      ...
      if (USE_PYTHON_BACKEND) {
        try {
          const b = await body(req);
          const pyRes = await proxyToPython('/api/tasks', 'POST', {
            department: b.dept,
            text: b.text,
            model: b.model,
            effort: b.effort
          ...
          console.error('[MIGRATION] Python backend error:', e.message);
          return json(res, 500, { error: 'Backend unavailable', detail: e.message });
        }
      }
    ...
    // [MIGRATION] Proxy task status GET to Python backend if enabled
    if (m && req.method === 'GET' && !m[2] && USE_PYTHON_BACKEND) {
      try {
        const pyRes = await proxyToPython(`/api/tasks/${m[1]}`, 'GET');
        return json(res, pyRes.status, pyRes.body);
      } catch (e) {
        console.error('[MIGRATION] Python backend error:', e.message);
        return json(res, 500, { error: 'Backend unavailable', detail: e.message });
    ...
    // [MIGRATION] Proxy approve/reject to Python backend if enabled
    if (m && req.method === 'POST' && (m[2] === 'approve' || m[2] === 'reject') && USE_PYTHON_BACKEND) {
      try {
        const b = await body(req);
        const endpoint = m[2] === 'approve' ? 'approve' : 'reject';
        const pyRes = await proxyToPython(`/api/tasks/${m[1]}/${endpoint}`, 'POST', b);
        return json(res, pyRes.status, pyRes.body);
      } catch (e) {
        console.error('[MIGRATION] Python backend error:', e.message);
        return json(res, 500, { error: 'Backend unavailable', detail: e.message });
...
server.listen(cfg.port, () => {
  ...
  if (!USE_PYTHON_BACKEND) {
    ...
    console.log(`  routines: DISABLED (using Python backend for task execution)`);
  }
